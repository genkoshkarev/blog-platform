            ...
            addEventGallery: () => {
                ...
                images.forEach(image => {
                    ...
                    image.addEventListener("click", (event) => {
                        this.#views.img.preview.load(event.target.getAttribute('src'), event.target.alt);
                    });
                    image.addEventListener("keydown", (event) => {
                        if (event.code === "Enter") {
                            console.log(1, event.target.firstElementChild)
                            this.#views.img.preview.load(event.target.firstElementChild.getAttribute('src'), event.target.alt);
                        }
                    });
                });
            },
            ...
            preview: {
                ...
                add: async () => {
                    ...
                    if (!imageFile === '' || imageName === '') {
                        alert('Загрузка картинки невозможна. Загрузите картинку и добавьте название изображения.');
                        return;
                    }
                        ...
                        await fetch(req)
                            ...
                            .then(commit=>{
                                ...
                                if (commit['status'] === "success") {
                                    if (this.img.dataset.img) {
                                        console.log(commit);
                                        this.$id(this.img.dataset.img).src = `/img/load/${commit['src']}`;
